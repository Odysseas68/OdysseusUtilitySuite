...
21. Current focus is Phase 5 — Polish & Advanced Controls. Module-page migration is complete; preserve existing database wiring, `Refresh()` behavior, command delegation, and hover help while adding focused polish.
...
25. Preserve current page database wiring and `Refresh()` behavior. Future layout polish should not change SavedVariables formats or module engine logic.
...
27. For launcher/minimap UI, prefer broker-compatible LibDataBroker-1.1 + LibDBIcon-1.0 architecture over raw manual minimap button ownership.
