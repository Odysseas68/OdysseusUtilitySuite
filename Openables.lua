...
opContainer:SetClampedToScreen(true)
...
function OP.OpenMassAddFrame()
    ...
    maddFrame:SetClampedToScreen(true)
...
SlashCmdList["DAWNCRESTS"] = function()
    ...
    if not dcFrame then
        ...
        for i = 1, #entries do
            ...
            row.iconBtn = CreateFrame("Button", nil, dcFrame)
            row.iconBtn:SetSize(28, 28)
            row.iconBtn:SetPoint("TOPLEFT", 14, y - 2)
            row.iconBtn:SetScript("OnEnter", function(self)
                ...
                GameTooltip:AddLine(" ")
                GameTooltip:AddLine("Held: " .. entry.quantity, 0.8, 0.8, 0.8)
                GameTooltip:AddLine("Season earned: " .. entry.totalEarned .. " / " .. entry.maxQuantity, 0.8, 0.8, 0.8)
                GameTooltip:AddLine("Can still earn: " .. entry.canGet, entry.canGet == 0 and 1 or 0, entry.canGet == 0 and 0 or 1, 0)
                GameTooltip:Show()
            end)
