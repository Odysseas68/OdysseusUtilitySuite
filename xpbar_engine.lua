...
function OUS.ApplyFonts()
    ...
    if LSM then fontPath = LSM:Fetch("font", OdysseusDB.xpBar.xpFont) or fontPath end
...
function OUS.FadeBarsTo(targetAlpha)
    ...
    local currentAlpha = xpBar:GetAlpha(); local step = (targetAlpha - currentAlpha) / 10; if step == 0 then return end
    local count = 0
    Session.fadeTicker = C_Timer.NewTicker(0.02, function()
        count = count + 1; local newAlpha = currentAlpha + (step * count)
...
function OUS.WakeBars()
    if not OdysseusDB or not OdysseusDB.xpBar.autoHide then OUS.FadeBarsTo((OdysseusDB and OdysseusDB.xpBar.activeAlpha or 100) / 100); return end
    if Session.sleepTimer then Session.sleepTimer:Cancel(); Session.sleepTimer = nil end
...
function OUS.SleepBars()
    ...
    Session.sleepTimer = C_Timer.NewTimer(OdysseusDB.xpBar.fadeDelay, function()
        if not UnitAffectingCombat("player") and not xpBar:IsMouseOver() and not delveBar:IsMouseOver() and not Session.forceRepDisplay and not (OUS.favHoverFrame and OUS.favHoverFrame:IsMouseOver()) then OUS.FadeBarsTo(OdysseusDB.xpBar.fadedAlpha / 100) end
    end)
end

xpBar:HookScript("OnEnter", OUS.WakeBars); xpBar:HookScript("OnLeave", OUS.SleepBars)
delveBar:HookScript("OnEnter", OUS.WakeBars); delveBar:HookScript("OnLeave", OUS.SleepBars)

-- ==========================================
-- 2. WAYPOINT ASSISTANT (TOAST CLICK)
-- ==========================================
...
local function DebugPrint(msg)
    if Session.isDebugOn and OUS.LogDebug then OUS.LogDebug(msg) end
end
...
local function ParseXPText(template, curXP, maxXP, restXP, level, mLVL, ktl, isMaxed, compName)
    ...
    if isMaxed then str = string.gsub(str, "%[curXP%]/%[maxXP%]", "Max Level"); str = string.gsub(str, "%[curXP%]", "Max"); str = string.gsub(str, "%[maxXP%]", "Max"); str = string.gsub(str, "%[needXP%]", "0")
    else str = string.gsub(str, "%[curXP%]", OUS.FormatLargeNumber(curXP)); str = string.gsub(str, "%[maxXP%]", OUS.FormatLargeNumber(maxXP)); str = string.gsub(str, "%[needXP%]", OUS.FormatLargeNumber(needXP)) end
    str = string.gsub(str, "%[restXP%]", OUS.FormatLargeNumber(restXP)); str = string.gsub(str, "%[curPC%]", curPC); str = string.gsub(str, "%[needPC%]", needPC); str = string.gsub(str, "%[restPC%]", restPC); str = string.gsub(str, "%[pLVL%]", level); str = string.gsub(str, "%[nLVL%]", level + 1); str = string.gsub(str, "%[mLVL%]", mLVL); str = string.gsub(str, "%[restLVL%]", restLVL); str = string.gsub(str, "%[KTL%]", ktl or "?"); str = string.gsub(str, "%[BTL%]", btl)
...
local function ParseRepText(template, name, standingText, curRep, maxRep, isMaxed)
    ...
    if isMaxed then str = string.gsub(str, "%[curRep%]/%[maxRep%]", "Max Level"); str = string.gsub(str, "%[curRep%]", "Max"); str = string.gsub(str, "%[maxRep%]", "Max"); str = string.gsub(str, "%[needRep%]", "0")
    else str = string.gsub(str, "%[curRep%]", OUS.FormatLargeNumber(curRep)); str = string.gsub(str, "%[maxRep%]", OUS.FormatLargeNumber(maxRep)); str = string.gsub(str, "%[needRep%]", OUS.FormatLargeNumber(needRep)) end
...
local function GetActiveDelveCompanion()
    ...
    local _, _, _, _, _, _, _, instanceID = GetInstanceInfo(); local uiMapID = C_Map.GetBestMapForUnit("player") or 0
...
local function GetFactionDetails(factionID)
    ...
    local name = data.name; local reaction = data.reaction or 4; local standingText = GetText("FACTION_STANDING_LABEL" .. reaction) or "Neutral"
    local curRep, maxRep = 0, 1; local isMaxed, hasRewardPending = false, false; local iconPath = "Interface\\Icons\\Achievement_Reputation_01"; local textureKit = nil
    
    if C_MajorFactions and C_MajorFactions.GetMajorFactionData then
        local majorData = C_MajorFactions.GetMajorFactionData(factionID)
        if majorData then
            standingText = "Renown " .. majorData.renownLevel; curRep = majorData.renownReputationEarned or 0; maxRep = majorData.renownLevelThreshold or 1
            if majorData.textureKit then textureKit = majorData.textureKit; iconPath = "Interface\\Icons\\UI_MajorFaction_" .. textureKit end
            if C_MajorFactions.HasMaximumRenown(factionID) then isMaxed = true; standingText = "Max Renown"; curRep = 1; maxRep = 1 end
        end
    end
    ...
    if not isMaxed and C_GossipInfo and C_GossipInfo.GetFriendshipReputation then
        ...
        if repInfo and repInfo.friendshipFactionID > 0 then
            ...
            if rankInfo and rankInfo.currentLevel then standingText = "Rank " .. rankInfo.currentLevel; if rankInfo.currentLevel >= rankInfo.maxLevel then isMaxed = true; standingText = "Max Rank"; curRep = 1; maxRep = 1 else curRep = repInfo.standing - repInfo.reactionThreshold; maxRep = repInfo.nextThreshold - repInfo.reactionThreshold end end
        end
    end
    
    if curRep == 0 and maxRep == 1 and data.currentStanding then
        if data.currentValue then curRep, maxRep = data.currentValue, data.maxValue else curRep = data.currentStanding - data.currentReactionThreshold; maxRep = data.nextReactionThreshold - data.currentReactionThreshold end
    ...
    if isMaxed and C_Reputation.IsFactionParagon(factionID) then
        ...
        if currentValue and threshold and threshold > 0 then isMaxed = false; curRep = currentValue % threshold; maxRep = threshold; hasRewardPending = hasReward; if hasRewardPending then standingText = "Paragon (Reward Ready!)"; if curRep == 0 then curRep = maxRep end else standingText = "Paragon" end end
    end
    ...
    if IsPlayerInDelve() then
        ...
        if C_GossipInfo and C_GossipInfo.GetFriendshipReputation then
            ...
            if repInfo and repInfo.friendshipFactionID > 0 then
                ...
                if rankInfo and rankInfo.currentLevel then cLvl = rankInfo.currentLevel; if rankInfo.currentLevel >= rankInfo.maxLevel then isMaxed = true; cMax = 1; cXP = 1 else cXP = repInfo.standing - repInfo.reactionThreshold; cMax = repInfo.nextThreshold - repInfo.reactionThreshold end end
            end
        end
        local jRep, jMax = 0, 1; local journeyData = C_Reputation.GetFactionDataByID(db.journeyID)
        ...
        if Session.isTestingDelve and cMax <= 1 and jMax <= 1 then cXP, cMax, cLvl = 25000, 83000, 15; jRep, jMax = 1200, 5000; isMaxed = false end
...
function OUS.UpdateBar()
    if not OdysseusDB or not OdysseusDB.xpBar then return end
    local db = OdysseusDB.xpBar
    local playerLevel = UnitLevel("player"); local maxExpansionLevel = GetMaxPlayerLevel and GetMaxPlayerLevel() or 80; local isMaxLevel = (playerLevel >= maxExpansionLevel) or (IsXPUserDisabled and IsXPUserDisabled())
    
    local targetFactionID = nil
    if Session.forceRepDisplay and Session.lastGainedFactionName then
        if C_Reputation and C_Reputation.GetNumFactions then
            for i = 1, C_Reputation.GetNumFactions() do local data = C_Reputation.GetFactionDataByIndex(i); if data and data.name == Session.lastGainedFactionName and not data.isHeader then targetFactionID = data.factionID; break end end
        end
    end
    if not targetFactionID then local w = C_Reputation.GetWatchedFactionData(); if w then targetFactionID = w.factionID end end

    if not isMaxLevel and not (Session.forceRepDisplay and targetFactionID) then
        local curXP, maxXP = UnitXP("player"), UnitXPMax("player"); local restXP = GetXPExhaustion() or 0; local ktl = "?"
        ...
        xpBar.text:SetText(ParseXPText(db.xpTemplate, curXP, maxXP, restXP, playerLevel, maxExpansionLevel, ktl, false, nil)); xpBar:Show()
    else
        ...
        if targetFactionID then
            ...
            if info then
                ...
                xpBar.text:SetText(ParseRepText(db.repTemplate, info.name, info.standingText, info.curRep, info.maxRep, info.isMaxed)); xpBar:Show()
            else xpBar:Hide() end
        else xpBar:Hide() end
    end
    OUS.UpdateDelveBar()
...
local function TriggerAggressiveDelveCheck()
    ...
    Session.delveCheckTicker = C_Timer.NewTicker(1, function()
        ...
        if IsPlayerInDelve() or checks >= 10 then 
            OUS.WakeBars(); OUS.UpdateBar(); OUS.SleepBars()
            if Session.delveCheckTicker then Session.delveCheckTicker:Cancel() end 
        end
    end)
end
...
factionSelectFrame:SetBackdrop({ bgFile = "Interface\\ChatFrame\\ChatFrameBackground", edgeFile = "Interface\\Tooltips\\UI-Tooltip-Border", tile = false, edgeSize = 16, insets = { left = 4, right = 4, top = 4, bottom = 4 }}); factionSelectFrame:SetBackdropColor(0.07, 0.05, 0.1, 0.98); factionSelectFrame:SetBackdropBorderColor(0.5, 0.3, 0.7, 1); factionSelectFrame:Hide()
factionSelectFrame:SetMovable(true); factionSelectFrame:EnableMouse(true); factionSelectFrame:RegisterForDrag("LeftButton"); factionSelectFrame:SetScript("OnDragStart", factionSelectFrame.StartMoving); factionSelectFrame:SetScript("OnDragStop", factionSelectFrame.StopMovingOrSizing); tinsert(UISpecialFrames, factionSelectFrame:GetName())
factionSelectFrame.headerBg = factionSelectFrame:CreateTexture(nil, "BACKGROUND", nil, 2); factionSelectFrame.headerBg:SetPoint("TOPLEFT", 4, -4); factionSelectFrame.headerBg:SetPoint("TOPRIGHT", -4, -4); factionSelectFrame.headerBg:SetHeight(30); factionSelectFrame.headerBg:SetColorTexture(1, 1, 1, 1); factionSelectFrame.headerBg:SetGradient("HORIZONTAL", CreateColor(0.3, 0.1, 0.5, 0.8), CreateColor(0.07, 0.05, 0.1, 0.8))
...
local function RefreshFactionSelectTree()
    ...
    local yOffset = 0; local numFactions = C_Reputation.GetNumFactions()
    for i = 1, numFactions do
        local data = C_Reputation.GetFactionDataByIndex(i)
        if data and not data.isHidden then
            local btn = fsButtons[i]
            if not btn then
                btn = CreateFrame("Button", nil, fsScrollChild, "BackdropTemplate"); btn:SetSize(380, 26)
                ...
                btn.wbIcon = btn:CreateTexture(nil, "OVERLAY"); btn.wbIcon:SetSize(14, 14); local success = pcall(function() btn.wbIcon:SetAtlas("warbands-icon") end); if not success then btn.wbIcon:SetTexture("Interface\\Icons\\Achievement_GuildPerk_EverybodysFriend") end
            ...
            btn.data = data; btn.check:SetChecked(tempFavorites[data.factionID] == true)
            local indent = data.isHeader and 5 or 25; if data.isHeader and data.isHeaderWithRep then indent = 15 end
            ...
            if data.isHeader and not data.isHeaderWithRep then
                ...
                if info then
                    ...
                    local displayCur = info.isMaxed and info.maxRep or info.curRep; btn.bar.txt:SetText(OUS.FormatLargeNumber(displayCur) .. " / " .. OUS.FormatLargeNumber(info.maxRep))
...
fsSaveBtn:SetScript("OnClick", function() if not OdysseusDB.xpBar.favFactions then OdysseusDB.xpBar.favFactions = {} end; OdysseusDB.xpBar.favFactions = OUS.DeepCopyTable(tempFavorites); factionSelectFrame:Hide() end)

OUS.favHoverFrame = CreateFrame("Frame", "OdysseusFavRepFrame", UIParent, "BackdropTemplate")
local favFrame = OUS.favHoverFrame
favFrame:SetSize(400, 200); favFrame:SetFrameStrata("TOOLTIP"); favFrame:SetBackdrop({ bgFile = "Interface\\ChatFrame\\ChatFrameBackground", edgeFile = "Interface\\Tooltips\\UI-Tooltip-Border", tile = false, edgeSize = 14, insets = { left = 3, right = 3, top = 3, bottom = 3 }}); favFrame:SetBackdropColor(0.05, 0.03, 0.08, 0.95); favFrame:SetBackdropBorderColor(0.6, 0.2, 0.8, 1); favFrame:Hide()
                    ...
                    if info then
                        ...
                        if not row then
                            ...
                            row:SetScript("OnEnter", function(self)
                                self:SetBackdropColor(0.2, 0.15, 0.3, 0.8); self:SetBackdropBorderColor(0.6, 0.2, 0.8, 1); OUS.WakeBars()
                            ...
                            row:SetScript("OnLeave", function(self) self:SetBackdropColor(0, 0, 0, 0.4); self:SetBackdropBorderColor(0.2, 0.2, 0.2, 1); GameTooltip:Hide(); C_Timer.After(0.1, function() if not favFrame:IsMouseOver() and not xpBar:IsMouseOver() then favFrame:Hide(); OUS.SleepBars() end end) end)
                            favRows[index] = row
                        end
                        ...
                        local displayCur = info.isMaxed and info.maxRep or info.curRep; row.bar.txt:SetText(OUS.FormatLargeNumber(displayCur) .. " / " .. OUS.FormatLargeNumber(info.maxRep))
                        ...
                        row:SetScript("OnClick", function(self, button)
                            if button == "RightButton" then
                                ...
                                if cIndex then C_Reputation.SetWatchedFactionByIndex(cIndex); Session.lastGainedFactionName = nil; Session.forceRepDisplay = false; if OUS.UpdateBar then OUS.UpdateBar() end; favFrame:Hide() end
                            end
                        end)
...
xpBar:SetScript("OnMouseUp", function(self, button)
    if button == "RightButton" then
        ...
        if open then if factionSelectFrame:IsShown() then factionSelectFrame:Hide() else tempFavorites = OUS.DeepCopyTable(OdysseusDB.xpBar.favFactions or {}); RefreshFactionSelectTree(); factionSelectFrame:Show() end end
    end
end)

local function CheckHideFavFrame() if not favFrame:IsMouseOver() and not xpBar:IsMouseOver() then favFrame:Hide(); OUS.SleepBars() end end

xpBar:HookScript("OnEnter", function()
    OUS.WakeBars(); RefreshHoverFavorites()
    local hasFavs = false
...
f:SetScript("OnEvent", function(self, event, arg1)
    if event == "ADDON_LOADED" and arg1 == addonName then
        ...
        for k, v in pairs(OUS.defaults) do if OdysseusDB.xpBar[k] == nil or (type(v) == "string" and OdysseusDB.xpBar[k] == "") then if type(v) == "table" then OdysseusDB.xpBar[k] = OUS.DeepCopyTable(v) else OdysseusDB.xpBar[k] = v end end end
        if OUS.BuildXPConfigUI then OUS.BuildXPConfigUI() end; OUS.ApplyFonts()
        ...
        OUS.ApplyDimensions(); Session.lastXP, Session.lastMaxXP = UnitXP("player"), UnitXPMax("player"); OUS.WakeBars(); OUS.SleepBars()
    elseif event == "PLAYER_ENTERING_WORLD" or event == "ZONE_CHANGED_NEW_AREA" then
        ApplyBlizzardKiller(); TriggerAggressiveDelveCheck(); ScanFactionsForPopups(true); OUS.WakeBars(); OUS.SleepBars()
    ...
    elseif event == "PLAYER_XP_UPDATE" then
        ...
        if currentXP > Session.lastXP and maxXP == Session.lastMaxXP then Session.lastXPGain = currentXP - Session.lastXP; Session.sessionXP = Session.sessionXP + Session.lastXPGain elseif currentXP < Session.lastXP or maxXP > Session.lastMaxXP then Session.lastXPGain = (Session.lastMaxXP - Session.lastXP) + currentXP; Session.sessionXP = Session.sessionXP + Session.lastXPGain end
        Session.lastXP, Session.lastMaxXP = currentXP, maxXP; if Session.forceRepDisplay then Session.forceRepDisplay = false; if Session.repTimer then Session.repTimer:Cancel() end end; OUS.WakeBars(); OUS.UpdateBar(); OUS.SleepBars()
    elseif event == "UPDATE_FACTION" then
        ...
        if not isMaxLevel then Session.forceRepDisplay = true; if Session.repTimer then Session.repTimer:Cancel() end; Session.repTimer = C_Timer.NewTimer(OdysseusDB.xpBar.repDisplayTime or 15, function() Session.forceRepDisplay = false; OUS.WakeBars(); OUS.UpdateBar(); OUS.SleepBars() end) end
        OUS.WakeBars(); OUS.UpdateBar(); OUS.SleepBars()
    elseif event == "CHAT_MSG_COMBAT_FACTION_CHANGE" or event == "CHAT_MSG_SYSTEM" then
        ...
        local msg = string.gsub(safeString, "|c%x%x%x%x%x%x%x%x", ""); msg = string.gsub(msg, "|r", ""); msg = string.gsub(msg, "|H.-|h(.-)|h", "%1")
        ...
        if faction and amount then faction = string.gsub(faction, "[%[%]%.]", ""); faction = string.gsub(faction, "^%s+", ""); faction = string.gsub(faction, "%s+$", ""); Session.sessionRep[faction] = (Session.sessionRep[faction] or 0) + tonumber(amount); Session.lastGainedFactionName = faction end
    elseif event == "PLAYER_REGEN_DISABLED" then OUS.WakeBars() elseif event == "PLAYER_REGEN_ENABLED" then OUS.SleepBars() end
end)