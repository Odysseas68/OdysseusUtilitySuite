...
- The frame displays Experience Gained, Reputation Breakdown, Gold Gained, Gold Spent, Repairs, Guild Repairs, Junk quantity/value, and Midnight Season 2 Mistcrests, and dynamically reflows when configured sections or currencies are hidden. XP Bar configuration exposes Show Experience, Show Reputation, Show Gold, Show Repairs, Show Currencies, and one tracking control for each built-in Mistcrest. Guild Repairs follows repair visibility; Junk follows gold visibility.
...
- Gold Gained and Gold Spent are gross personal-wallet income and spending. Utilities reports known OUS merchant transactions through `OUS.SessionStats.RecordKnownMerchantTransaction()`: junk income and personal repairs count immediately and enqueue signed wallet effects; guild repairs only update their informational breakdown. Personal Repairs is a subset of Gold Spent; Junk Gold is a subset of Gold Gained, and Junk Items counts physical stack quantities. The fallback repair observer uses a `RepairAllItems` post-hook without replacing Blizzard's function.
